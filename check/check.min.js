...
c.minInclusive});c.maxInclusive&&d>c.maxInclusive&&g.push({status:986,statusText:a+": double value must be smaller than "+c.maxInclusive});break;case "xs:string":Array.isArray(d)?"number"==typeof c.maxLength&&0>m.dontCheck.indexOf("text.length")&&d.forEach(function(k){k.text.length>c.maxLength&&g.push({status:986,statusText:a+": length of string value must not exceed "+c.maxLength})}):g.push({status:986,statusText:a+": value must be a list of multi-language objects"})}}}))}function x(b,f,a){f.forEach(function(c){function d(h){k=
k.concat(h.propertyClasses||[]);"object"==typeof h["extends"]&&d(p(b,h["extends"]))}var k=[],l,n,t=p(b,c[a]);t&&(d(t),Array.isArray(c.properties)&&c.properties.forEach(function(h){l="property with class '"+h["class"].id+"' of instance with identifier '"+c.id+"'";q(k,h["class"])&&g.push({status:987,statusText:l+": class must be listed with the instance class or a parent instance class"});n=p(e.propertyClasses,h["class"]);"object"==typeof n?v(n,h.values,l):g.push({status:987,statusText:l+" must reference a valid propertyClass"})}))})}
function y(b,f,a){Array.isArray(f)&&f.forEach(function(c){q(b,c.resource)&&g.push({status:988,statusText:"hierarchy node with identifier '"+c.id+"' must reference a valid resource"});y(b,c.nodes,a+1)})}function z(b){return{id:b.id,revision:b.revision}}function w(b){if(Array.isArray(b)){for(var f=1<b.length,a=b.length-1;-1<a;a--)if("string"!=typeof b[a].text||f&&("string"!=typeof b[a].language||2>b[a].language.length))return!1;return!0}return!1}function p(b,f){var a=b.filter(function(c){return c.id==
f.id});if(!(1>a.length)){if(1==a.length&&!a[0].revision)return f.revision?void 0:a[0];a.sort(function(c,d){return d.changedAt-c.changedAt});if(!f.revision)return a[0];a=a.filter(function(c){return c.revision==f.revision});if(0<a.length)return a[0]}}function q(b,f){return void 0==p(b,f)}function A(b){var f="";b.forEach(function(a){f+=(f.length?",\n":"")+a.statusText+" ("+status+")"});return f}if(e.specifVersion)return{status:970,statusText:"This constraint checker does not support any SpecIF version below 1.1"};
switch(e.$schema){case "https://specif.de/v1.0/schema.json":case "https://json.schemastore.org/specif-1.0.json":return{status:970,statusText:"This constraint checker does not support SpecIF version 1.0"}}"object"!=typeof m&&(m={});Array.isArray(m.dontCheck)||(m.dontCheck=[]);var g=[];(function(){function b(a){if(Array.isArray(a)&&0<a.length)for(var c,d=a.length-1;-1<d;d--)if(void 0!=a[d].id){a:{for(var k=a[d],l=k.revision||"",n=f.length-1;-1<n;n--)if(c=f[n].revision||"",f[n].id==k.id&&(c==l||""==
c||""==l)){c=!0;break a}c=!1}if(c)return a[d];if((c=b(a[d].enumeration))||(c=b(a[d].nodes)))return c;f.push(z(a[d]))}}var f=[z(e)];[{list:e.dataTypes,name:"dataType or enumerated value"},{list:e.propertyClasses,name:"propertyClass"},{list:e.resourceClasses,name:"resourceClass"},{list:e.statementClasses,name:"statementClass"},{list:e.resources,name:"resource"},{list:e.statements,name:"statement"},{list:e.hierarchies,name:"hierarchy or node"},{list:e.files,name:"file"}].forEach(function(a){var c=b(a.list);
c&&g.push({status:973,statusText:"key of "+a.name+" '"+c.id+"' with revision "+c.revision+" is not unique"})})})();(function(){e.dataTypes.forEach(function(b){switch(b.type){case "xs:double":case "xs:integer":"number"==typeof b.minInclusive&&"number"==typeof b.maxInclusive&&b.minInclusive+1>b.maxInclusive&&g.push({status:974,statusText:"dataType '"+b.id+"': minInclusive must be smaller or equal than maxInclusive"});case "xs:dateTime":case "xs:anyURI":case "xs:string":if(Array.isArray(b.enumeration))if(0<
...
f=e.resourceClasses.concat(e.statementClasses);e.statements.forEach(function(a,c){var d=p(b,a.subject),k=p(b,a.object);!d&&0>m.dontCheck.indexOf("statement.subject")&&g.push({status:980,statusText:"subject of statement["+c+"] with identifier '"+a.id+"' must reference a valid resource or statement"});!k&&0>m.dontCheck.indexOf("statement.object")&&g.push({status:980,statusText:"object of statement["+c+"] with identifier '"+a.id+"' must reference a valid resource or statement"});var l=p(e.statementClasses,
a["class"]);if(Array.isArray(l.subjectClasses)&&d){var n=[];l.subjectClasses.forEach(function(h){(h=p(f,h))&&n.push(h)});q(n,d["class"])&&g.push({status:981,statusText:"the subject of statement["+c+"] with identifier '"+a.id+"' has a class which is not listed in the subjectClasses of the statement's class"})}if(Array.isArray(l.objectClasses)&&k){var t=[];l.objectClasses.forEach(function(h){(h=p(f,h))&&t.push(h)});q(t,k[["class"]])&&g.push({status:981,statusText:"the object of statement["+c+"] with identifier '"+
